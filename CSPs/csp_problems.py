...
def solve_planes(planes_problem, algo, allsolns,
    ...
    for flight in planes_problem.flights:
        constraint_list.append(NValuesConstraint("SingleFlight_{}".format(flight), all_vars, [flight], 1, 1))
    ...
    for plane in planes_problem.planes:
        ...
        for f1, f2 in planes_problem.can_follow:
            if f1 in planes_problem.can_fly(plane) and f2 in planes_problem.can_fly(plane):
        ...
        for i in range(len(plane_vars) - 1):
            ...
            constraint_list.append(TableConstraint("FeasibleSeq_{}".format(i), scope, feasible_set))
    ...
    for plane in planes_problem.planes:
        ...
        for i in range(0, len(plane_vars) + 1 - planes_problem.min_maintenance_frequency, 1):
            scope = plane_vars[i: i + planes_problem.min_maintenance_frequency]
            constraint_list.append(
                NValuesConstraint("planeMaintenanceConstrain_{}{}".format(flight, i), scope, plane_maintenance, 1,
