...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        scope = [qi, qj]
        satAssign = []
        for queen_i in qi.domain():
            for queen_j in qj.domain():
                if (queen_i - queen_j) != (self.i - self.j) and (queen_i - queen_j) != -(self.i - self.j) and queen_j != queen_i:
                    satAssign.append([queen_i, queen_j])
        TableConstraint.__init__(self, name, scope, satAssign)
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        required_assign_counter = 0
        for v in self.scope():
            if v.getValue() == self.get_required():
                required_assign_counter += 1
        # (lower_bound <= #of variables assigned 'required_value' <= upper_bound)
        return self._lb <= required_assign_counter <= self._ub
    ...
    def hasSupport(self, var, val):
        ...
        def fullTest(l):
            ...
            for var, val in l:
                if val in self._required:
                    sum += 1
            return (sum >= self._lb and sum <= self._ub)

        def partialTest(l):
            sum = 0
            remain = len(self.scope())
            for var, val in l:
                remain -= 1
                if val in self._required:
                    sum += 1
                if sum + remain < self._lb:
                    return False
        ...
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], fullTest, partialTest)
        return x
